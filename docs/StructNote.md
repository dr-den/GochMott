...
заголовком нигде не стоят — по ним прямой поиск ЧЕ→РУ ничего не найдёт (останется
примерный, он ищет по тем же заголовкам). Если это понадобится вернуть, правильное
место — **отдельный блок** «встречается в переводах» под основной выдачей, а не
подмешивание русских статей в общий список.
