...
Снимает их `Marks` в `model/Models.kt` — по одному проходу на строку, отдельно для
долготы и для ударения. Обратной операции нет и не нужно: размеченная строка и есть
источник, а ключи поиска (`*_norm`) лежат в базе уже очищенными.
