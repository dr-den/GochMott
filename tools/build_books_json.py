#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Собирает вводную часть словарей 1978, 1997, 2012 и 2017 в assets/books/.

Мациев собирается отдельно (`extract_about.py` -> `assets/about.json`): у него
есть ещё список сокращений и алфавит, а у этих книг их нет.
...
На выходе:
    ...
    app/src/main/assets/books/aslakhanov2012.json вводная часть 2012
...
# У Аслаханова предисловие напечатано на обоих языках, а слово редактора
# («Редакторера») — только по-чеченски. Файл тот же, что читает парсер
# (`rawSources/aslakhanov2012.odt`): у второй копии книги с длинным именем
# другая нумерация абзацев.

ASLAKHANOV = {
    'code': 'aslakhanov2012',
    'odt': 'aslakhanov2012.odt',
    'title_ce': 'Оьрсийн-нохчийн спортивни терминийн а, дешнийн цхьаьнакхетарийн а словарь',
    'sections': [
        {'id': 'preface', 'title': {'ru': 'Предисловие', 'ce': 'Дешхьалхе'},
         'ru': range(134, 143), 'ce': range(145, 152)},
        {'id': 'editor', 'title': {'ru': 'От редактора', 'ce': 'Редакторера'},
         'ru': range(0, 0), 'ce': range(155, 161)},
    ],
}
...
NOTES = {
    ...
    'aslakhanov2012': [
        'Книга свёрстана в две колонки, и при переносе в приложение статьи '
        'собирались из разорванных строк: где граница статьи в вёрстке не видна, '
        'она проведена по догадке. В книге есть и свои опечатки и ошибки '
        'согласования. Поэтому плашка этого словаря в выдаче жёлтая, а сам словарь '
        'можно отключить фильтром словарей в верхней панели.',
        'Словарь разбит на разделы по видам спорта, и один термин часто стоит '
        'в нескольких из них: «бросок» есть и в борьбе, и в дзюдо, '
        'и в лёгкой атлетике. '
        'В приложении раздел показан пометой при заглавном слове — «футбол», '
        '«бокс», — а одинаковые заглавные слова из разных разделов стали '
        'омонимами.',
        'После чеченского перевода книга даёт в скобках его склонение и классный '
        'показатель: «амплитуда (амплитудан, амплитудана, амплитудо, амплитуде, й)». '
        'Приложение оставляет в строке перевода само слово и класс, а падежные '
        'формы не показывает.',
        'Ударений в русских словах и знаков долготы в чеченских эта книга не '
        'печатает, поэтому переключатели «Ударения в переводах» и «Долгота '
        'гласных» на её статьи не влияют.',
    ],
}
...
def main():
    ...
    for spec in (KARASAEV, MATH, COMP, ASLAKHANOV):
        code = spec['code']
        if code not in meta:
