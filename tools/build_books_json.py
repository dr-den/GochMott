#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Собирает вводную часть словарей 1978, 1997, 2008, 2012 и 2017 в assets/books/.
...
На выходе:
    ...
    app/src/main/assets/books/law2008.json        вводная часть 2008
...
# У Абдурашидова предисловие («Дешхьалхе») напечатано только по-чеченски,
# а «Построение словаря» — только по-русски: книга двуязычная, но вводную часть
# каждый раздел даёт на своём языке.

LAW = {
    'code': 'law2008',
    'odt': 'law2008.odt',
    'title_ce': 'Нохчийн-оьрсийн Ӏедалца йоьзначу терминийн дошам',
    'sections': [
        {'id': 'preface', 'title': {'ru': 'Предисловие', 'ce': 'Дешхьалхе'},
         'ru': range(0, 0), 'ce': range(135, 146)},
        {'id': 'structure', 'title': {'ru': 'Построение словаря', 'ce': ''},
         'ru': range(164, 188), 'ce': range(0, 0)},
    ],
}
...
NOTES = {
    ...
    'law2008': [
        'Книга двуязычная, и в приложении это два словаря с одной плашкой: '
        'чеченско-русский и русско-чеченский. Поэтому чеченское слово ищется '
        'в ней напрямую, а не только по переводу.',
        'Внутри статьи книга заменяет неизменяемую часть заглавного слова '
        'тильдой: «тоьшалла (д) протокол; дегӀах хьажаран ~ протокол личного '
        'обыска». В приложении такие сочетания собраны целиком, поэтому их можно '
        'искать и копировать как обычные слова.',
        'Классный показатель стоит там же, где в книге: в чеченско-русской части — '
        'при заглавном слове, в русско-чеченской — при переводе. Падежные формы '
        'чеченского слова книга печатает в самой статье, а приложение их пока '
        'не показывает; пометы языка-источника («лат.», «фр.») — тоже.',
        'Вёрстка книги местами теряет начертание, по которому разбирается статья, '
        'и 128 статей восстановлены по языку текста; в книге есть опечатки '
        'в заглавных словах. Поэтому плашка словаря жёлтая, а сам он отключается '
        'фильтром словарей в верхней панели.',
...
def main():
    ...
    for spec in (KARASAEV, MATH, COMP, LAW, ASLAKHANOV):
        code = spec['code']
        if code not in meta:
