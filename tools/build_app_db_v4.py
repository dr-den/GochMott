...
DB_USER_VERSION = 4   # держать синхронно с DatabaseHelper.EXPECTED_DB_VERSION
...
def build(db_path, sources, class_forms='safe', want_fts=True, want_links=False):
            ...
            for k, s in enumerate(senses or []):
                ...
                for j, g in enumerate(s.get('glosses') or []):
                    ...
                    db.execute(
                        ...
                        'text_norm,text_fold,sep,note,gov,labels)'
                        ' VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        (gid, did, sid, lemma, j, ltgt, gtext, gnorm, gfold,
                         g.get('sep'), g.get('note'), g.get('gov'), J(g.get('labels'))))
        ...
        for k, d in enumerate(entries, 1):
            ...
            db.execute(
                ...
                (lid, did, lsrc, d['_slug'], hw, hw_norm, hw_fold, d.get('homonym'),
                 ...
                 d.get('obj_num'), d.get('subj_num'), JO(d.get('gram')),
    ...
    if want_links and len(sources) > 1:
        link_stats, link_report = build_links(db, ids)
...
def build_links(db, ids):
    ...
    # переводы леммы — для двуязычной сверки на шаге 3
    ...
    def emit(a, b, method, conf):
        ...
        db.execute('INSERT INTO lemma_links(id,a_lemma_id,b_lemma_id,a_dict_id,'
                   'b_dict_id,lang,method,confidence,conflict)'
                   ' VALUES(?,?,?,?,?,?,?,?,?)',
                   (ids['link'], a, b, ra[0], rb[0], ra[1], method, conf, J(conflict)))
        stats[method] += 1
        if conflict:
            stats['conflicts'] += 1
            report.append((ra[2], rb[2], code_of[ra[0]], code_of[rb[0]],
                           method, ','.join(conflict),
                           ra[6], rb[6], ra[7], rb[7]))
...
def main(argv=None):
    ...
    if args.links_report and info['link_report']:
        with open(args.links_report, 'w', encoding='utf-8') as f:
            f.write('a_headword\tb_headword\ta_dict\tb_dict\tmethod\tconflict'
