 ...
 * читать русский перевод, чёрточка долготы передаёт чеченскую фонетику, которую
 * современное письмо не записывает, а словарь Мациева записывает.
 *
 * По умолчанию показаны оба — словарь для того и разбирался заново, чтобы долготы
 * не терялись. Если на каком-то шрифте комбинирующая U+0303 над кириллицей ляжет
 * криво, выключается она здесь одним флагом, без пересборки базы: в БД лежит
 * размеченный текст, а ключи поиска (`*_norm`) очищены отдельно и не зависят
 * от того, что показано.
 */
object Marks {
