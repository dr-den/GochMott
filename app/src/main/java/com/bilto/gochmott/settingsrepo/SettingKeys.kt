...
object SettingKeys {
    ...
    /**
     * Кэш подписи пустого экрана: `версия:размер|книг|чеченских слов|русских слов`.
     *
     * Считать это при каждом старте нельзя — запрос по русской стороне пробегает
     * весь обратный индекс на 93 тысячи строк. Словарь read-only и меняется только
     * целиком, поэтому сборка в первом поле — `PRAGMA user_version` и размер
     * файла — и есть признак годности кэша: не совпала — пересчитать. Одной
     * версии мало: базу пересобирают и без смены схемы.
     *
    ...
    /**
     * Отключённые в фильтре книги (`dicts.book`) через запятую.
     *
     * Хранятся именно ОТКЛЮЧЁННЫЕ: книга, которая придёт с новой базой, у
     * читателя сразу включена, а не молча спрятана.
     */
    val disabledBooks = SettingKey.Str("disabledBooks", "")
