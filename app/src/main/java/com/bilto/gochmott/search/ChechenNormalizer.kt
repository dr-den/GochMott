...
/**
 * ТОЧНЫЙ порт normalize_ce.py. Этим же кодом нормализованы ключи *_norm в dict.db,
 * поэтому пользовательский чеченский ввод ПЕРЕД запросом `WHERE form_norm = ?`
 * нужно прогонять ровно через ChechenNormalizer.normalize(). Любое расхождение с
